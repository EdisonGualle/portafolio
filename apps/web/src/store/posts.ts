...
export const usePostsStore = create<ListState & DetailState & Actions>((set) => ({
  items: [],
  loading: false,
  error: null,
  page: 1,
  ...
  async fetch(params) {
    ...
    try {
      const { data, meta } = await postsService.list({ per_page: 6, page: params?.page });
  ...
  async show(slug: string) {
    ...
    try {
      const data = await postsService.show(slug);
